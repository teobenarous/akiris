...
# https://github.com/andreweland/swemls/blob/main/generator/population.py
...
def choose_creatinine_for_aki(
    person, previous_results, epoch, aki_creatinine_multipliers
):
...
def generate_people(n, epoch, ages_filename, baseline_creatinine_multipliers):
    ...
    for _ in range(0, n):
        ...
        person.birthdate = datetime.date(
            epoch.year - age_years, epoch.month, epoch.day
        ) - datetime.timedelta(days=age_days)
        person.sex = random.choice([SEX_MALE, SEX_FEMALE])
        (
            person.creatinine_mu,
            person.creatinine_sigma,
            person.creatinine_multiplier,
        ) = _choose_creatinine_baseline(person, epoch, baseline_creatinine_multipliers)
...
def add_names(
    ...
    surnames = [line.strip() for line in open(surnames_filename)]
    female_forenames = [line.strip() for line in open(female_forenames_filename)]
    male_forenames = [line.strip() for line in open(male_forenames_filename)]
    for p in people:
        surname = random.choice(surnames)
        forename = random.choice(
            female_forenames if p.sex == SEX_FEMALE else male_forenames
        )
