# Adapted from the script written by Andrew Eland:
# https://github.com/andreweland/swemls/blob/main/generator/generator.py
...
# Empirically we need a mean of ~30 admissions to stabilize around
# 800 admitted patients
HOSPITAL_BEDS = 800
STAY_DAYS_DISTRIBUTION = statistics.NormalDist(HOSPITAL_BEDS / 40, 35)
MIN_MRN = 1_000_000 * 100
MAX_MRN = 2_000_000 * 100
...
class HospitalTimesSampler:
    def __init__(self):
        ...
        for minute in range(18 * 60, 24 * 60):
            self.weights[minute] = min(
                1.0, 1.0 / ((minute - (18 * 60) + 1) / falloff_factor)
            )
        for minute in range(0, 6 * 60):
            self.weights[(6 * 60) - minute - 1] = min(
                1.0, 1.0 / ((minute + 1) / falloff_factor)
            )
...
def generate_admit_discharge_events(people, times, days, mean_daily_admits):
    ...
    q.put((start, (EVENT_DAY_START, None, None)))
    now = None
    inpatients = set()
    while now is None or now < end:
        now, (event, mrn, detail) = q.get()
        yield now, (event, mrn, detail)
        if event == EVENT_DAY_START:
            q.put((now + datetime.timedelta(days=1), (EVENT_DAY_START, None, None)))
            for _ in range(0, int(admit_distribution.samples(1)[0])):
                while True:
                    person = random.choice(people)
                    if person.mrn not in inpatients:
                        break
                inpatients.add(person.mrn)
                admit_time = combine_date_time(now.date(), times.sample())
                q.put((admit_time, (EVENT_ADMIT, person.mrn, None)))
                discharge_time = admit_time
                while discharge_time <= admit_time:
                    days = int(STAY_DAYS_DISTRIBUTION.samples(1)[0])
                    discharge_time = combine_date_time(
                        now.date() + datetime.timedelta(days=days), times.sample()
                    )
                q.put((discharge_time, (EVENT_DISCHARGE, person.mrn, None)))
...
def add_blood_test_events(events, times, by_mrn):
    ...
    for e in events:
        ...
        while not q.empty():
            q_now, (q_event, q_mrn, q_detail) = q.get()
            if q_now < now:
                if q_mrn in inpatients:
                    person = by_mrn[q_mrn]
                    result = (
                        random.gauss(person.creatinine_mu, person.creatinine_sigma)
                        * person.creatinine_multiplier
                    )
                    yield q_now, (q_event, q_mrn, result)
                    if random.random() < REPEATED_BLOOD_TEST_PROBABILITY:
                        test_time = times.maybe_sample_after(q_now)
                        if test_time:
                            q.put((test_time, (EVENT_BLOOD_TEST, q_mrn, None)))
            else:
                q.put((q_now, (q_event, q_mrn, q_detail)))
                break
        yield now, (event, mrn, detail)
        if event == EVENT_DAY_START:
            for inpatient in inpatients:
                if random.random() < DAILY_BLOOD_TEST_PROBABILITY:
                    ...
                    if test_time:
                        q.put((test_time, (EVENT_BLOOD_TEST, inpatient, None)))
        elif event == EVENT_ADMIT:
            inpatients.add(mrn)
            if random.random() < ADMIT_BLOOD_TEST_PROBABILITY:
                test_time = times.maybe_sample_after(now)
                if test_time:
                    q.put((test_time, (EVENT_BLOOD_TEST, mrn, None)))
        elif event == EVENT_DISCHARGE:
            inpatients.remove(mrn)
    ...
    for e in events:
        ...
        if event == EVENT_BLOOD_TEST and now > start:
            ...
            for (age_begin, age_end), f in AKI_AGE_FACTORS:
                if age_begin <= age_years < age_end:
                    age_factor = f
                    break
            else:
                raise AssertionError()
            if (
                len(previous_results.get(mrn, [])) > 0
                and mrn not in had_aki
                and random.random() < BASE_AKI_PROBABILITY * age_factor
            ):
                result = population.choose_creatinine_for_aki(
                    person,
                    previous_results[person.mrn],
                    epoch,
                    generation_args.aki_creatinine_multipliers,
                )
...
def new_hl7_message(time, message_code, trigger_event):
    m = Message(
        f"{message_code}_{trigger_event}",
        version="2.5",
...
def output_training(events, by_mrn, epoch, filename):
    ...
    with open(filename, "w") as f:
        ...
        for mrn, results in results_by_mrn.items():
            person = by_mrn[mrn]
            row = [
                str(person.age_years(epoch)),
                "f" if person.sex == population.SEX_FEMALE else "m",
                "y" if mrn in akis else "n",
            ]
            row.extend(csv_results(results))
            while len(row) < len(headers):
                row.append("")
    ...
    with open(filename, "w") as f:
        ...
        for e in events:
            ...
            if event in (EVENT_BLOOD_TEST, EVENT_BLOOD_TEST_AKI):
                ...
                sex = (
                    nhs.SEX_FEMALE
                    if by_mrn[mrn].sex == population.SEX_FEMALE
                    else nhs.SEX_MALE
                )
...
AKI_OUTPUT_FILENAME = "ground_truth.csv"
TRAINING_OUTPUT_FILENAME = "dataset.csv"
...
class GenerationArgs:
    def __init__(self):
        ...
        self.baseline_creatinine_multipliers = (
            population.BASELINE_CREATININE_MULTIPLIERS
        )
...
def generate(generation_args, data_directory, output):
    ...
    people = population.generate_people(
        generation_args.population_size,
        epoch,
        population_filename,
        generation_args.baseline_creatinine_multipliers,
    )
    population.add_names(
        people, surnames_filename, female_forenames_filename, male_forenames_filename
    )
    ...
    adt_events = generate_admit_discharge_events(
        people,
        times,
        generation_args.days + generation_args.history,
        generation_args.mean_daily_admits,
    )
    all_events = add_blood_test_events(adt_events, times, by_mrn)
    aki_events = list(
        inject_aki_blood_test_events(
            generation_args,
            all_events,
            by_mrn,
            epoch,
            datetime.timedelta(days=generation_args.history),
        )
    )
    historical_events, following_events = wait_until(
        aki_events, epoch + datetime.timedelta(days=generation_args.history)
    )
...
def main():
    ...
    parser.add_argument("--data", default="data/assets")
