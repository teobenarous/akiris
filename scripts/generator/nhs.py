...
# https://github.com/andreweland/swemls/blob/main/generator/nhs.py
...
def has_aki(age, sex, test_results, now):
    ...
    if d < datetime.timedelta(days=365):
        within_7 = [
            result
            for (date, result) in test_results[:-1]
        ...
        within_365 = [
            ...
            for (date, result) in test_results[:-1]
            if now - date > datetime.timedelta(days=7)
            and now - date < datetime.timedelta(days=365)
        ]
        within_7_rv = 0.0
        if len(within_7) > 0:
            rv1 = min(within_7)
    ...
    def test_recent_test_result_with_aki(self):
        ...
        self.assertTrue(
            has_aki(
                42,
                SEX_MALE,
                [(now - datetime.timedelta(days=1), 104.0), (now, 300.0)],
                now,
            )
        )
    ...
    def test_recent_test_result_without_aki(self):
        ...
        self.assertFalse(
            has_aki(
                42,
                SEX_MALE,
                [(now - datetime.timedelta(days=1), 104.0), (now, 102.0)],
                now,
            )
        )
    ...
    def test_old_test_result_with_aki(self):
        ...
        self.assertTrue(
            has_aki(
                42,
                SEX_MALE,
                [(now - datetime.timedelta(days=30), 104.0), (now, 300.0)],
                now,
            )
        )
